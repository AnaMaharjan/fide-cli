    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "  - `list`, `get`, and `save` operate on hosted graphs owned by the current account in the selected workspace.",
          "  - `graph save --graph <key>` will reuse the local project graph definition with the same key when available.",
          "  - `graph save` projects shared graph metadata only; local connection settings stay in project `.fide/settings.json`.",
