...
/**
 * Parse a statement input/batch and return its deterministic root and count.
 */
export async function runGraphCheck(args: string[]): Promise<number> {
  const { flags } = parseArgs(args);
  if (hasFlag(flags, "help") || hasFlag(flags, "-h")) {
    console.log("Usage: fide graph check --in <input> [--format <json|jsonl|fsd>] [--fields <mask>] [--pretty|-p]");
    return 0;
  }
