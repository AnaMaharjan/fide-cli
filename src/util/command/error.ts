...
function statementsGuideNext(path = "/vocabulary"): Record<string, string> {
  return {
    guideCommand: "fide transformers guide",
  ...
  if (invalidEntityType) {
    ...
    return {
      ...
      error: {
        ...
        hint: value === "Text"
          ? "Use TextLiteral for plain text values. Run `fide transformers guide` to inspect valid entity types."
          : "Run `fide transformers guide` to inspect valid entity types.",
      },
  ...
  if (invalidReferenceType) {
    ...
    return {
      ...
      error: {
        ...
        hint: "Run `fide transformers guide` to inspect valid reference types.",
      },
      next: statementsGuideNext(value === "Text" ? "/vocabulary/definitions/text-literal" : "/vocabulary"),
    };
  }
